...
private:
  void ReAlloc(size_t newCapacity) {
    ...
    // 2. copy(wate of perf) so maybe move existing element in the new block of
    // memory
    // 3. delete

    // as we are designing low level it's better not use smart pointer as unique
    // ptr & shared pointer
...
public:
  ...
  Vector1 &operator=(const Vector1 &a); // copy assignmnet by =
